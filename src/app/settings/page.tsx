...
export default function SettingsPage() {
  ...
  const { hasPermission: allowDataUsage, setPermission: setAllowDataUsage } =
    usePermission("analytics");
  const [isDataUsageSheetOpen, setIsDataUsageSheetOpen] = React.useState(false);
  const [nearbyRecommendations, setNearbyRecommendations] =
    React.useState(true);
  const [isNearbySheetOpen, setIsNearbySheetOpen] = React.useState(false);
  const {
    hasPermission: cameraPermission,
    setPermission: setCameraPermission,
  } = usePermission("camera", true);
  ...
  const [pinStep, setPinStep] = React.useState<
    "create" | "confirm" | "disable"
  >("create");
  ...
  const handleDataUsageConfirm = (consent: boolean) => {
    if (consent) {
      ...
      toast({
        ...
        description: `Anonymous data sharing is now ${
          !allowDataUsage ? "enabled" : "disabled"
        }.`,
      });
    }
  ...
  const handleNearbyRecommendationsConfirm = (consent: boolean) => {
    if (consent) {
      ...
      toast({
        ...
        description: `Location access for recommendations is now ${
          !nearbyRecommendations ? "enabled" : "disabled"
        }.`,
      });
    }
  ...
  const getMaskedApiKey = () => {
    ...
    return `${geminiApiKey.slice(0, 4)}${"•".repeat(
      Math.min(geminiApiKey.length - 8, 20)
  ...
  const handleConfirmPin = async (confirmedPin: string[]) => {
    ...
    if (originalPin === confirmed) {
      ...
      toast({
        title: "PIN Created",
        description:
          "Two-factor authentication is now enabled. You'll need to enter this PIN when opening the app.",
      });
    } else {
      // PINs don't match
      setIsShaking(true);
    ...
    <div className="max-w-4xl mx-auto px-4 md:px-6 pt-20 md:pt-8 pb-24 md:pb-8">
      <div className="divide-y space-y-6">
        <SettingsSection title="Account & Security">
          <Sheet
            onOpenChange={(open) =>
              !open && (setIsCodeSent(false), form.reset())
            }
          >
          ...
          <SettingsItem
            ...
            description={
              tfaEnabled ? "PIN enabled" : "Protect your app with a PIN"
            }
          >
            <Switch
              id="tfa-switch"
          ...
          <Dialog open={isPinDialogOpen} onOpenChange={handlePinDialogClose}>
            <DialogContent className="sm:max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>
                  {pinStep === "create" &&
                    "Create a 4-digit PIN to protect your app. You'll need to enter this PIN when opening SugVoyage."}
                  {pinStep === "confirm" && "Re-enter your PIN to confirm."}
                  {pinStep === "disable" &&
                    "Enter your current PIN to disable two-factor authentication."}
                </DialogDescription>
              </DialogHeader>
            ...
            <SheetContent side="bottom" className="rounded-t-2xl">
              ...
              <div className="py-4">
                ...
                >
                  <Label
                    htmlFor="lang-en"
                    className="flex items-center justify-between p-4 border-b w-full cursor-pointer"
                  >
                    English
          ...
          <Sheet open={isApiKeySheetOpen} onOpenChange={setIsApiKeySheetOpen}>
              ...
              <div className="cursor-pointer">
                <SettingsItem
                  ...
                  description={
                    geminiApiKey
                      ? getMaskedApiKey()
                      : "Add your free API key for AI features"
                  }
                >
                  <ChevronRight className="h-5 w-5 text-muted-foreground" />
                </SettingsItem>
            ...
            <SheetContent side="bottom" className="rounded-t-2xl">
              <SheetHeader className="text-left">
                ...
                <SheetDescription>
                  Enter your free Gemini API key to use AI features like
                  itinerary generation, landmark scanning, and more. Get your
                  free key at{" "}
                  <a
          ...
          <Sheet>
            <SheetTrigger asChild>
              <div className="cursor-pointer">
                <SettingsItem
                  icon={Volume2}
                  title="Audio narration voice"
                  description={audioVoice}
                >
                  <ChevronRight className="h-5 w-5 text-muted-foreground" />
                </SettingsItem>
              </div>
            ...
            <SheetContent side="bottom" className="rounded-t-2xl">
              <SheetHeader className="text-left">
                ...
                <SheetDescription>
                  Select your preferred voice for AI-powered audio narrations.
                </SheetDescription>
              </SheetHeader>
              <div className="py-4">
                <RadioGroup
          ...
          <div className="py-2">
            ...
            <div className="space-y-1 divide-y">
              ...
              <SettingsItem icon={Bell} title="Nearby recommendations">
                <Switch
                  checked={nearbyRecommendations}
                  onCheckedChange={handleNearbyRecommendationsToggle}
                />
              </SettingsItem>
              <Sheet
                open={isNearbySheetOpen}
                onOpenChange={setIsNearbySheetOpen}
              >
                <SheetContent side="bottom" className="rounded-t-2xl">
                  <SheetHeader>
                  ...
                  <SheetFooter className="grid grid-cols-2 gap-4 pt-4">
                    <Button
                      variant="outline"
                      onClick={() => handleNearbyRecommendationsConfirm(false)}
                    >
                      No
                    </Button>
                    <Button
                      onClick={() => handleNearbyRecommendationsConfirm(true)}
                    >
                      Yes
                    </Button>
                  </SheetFooter>
        ...
        <SettingsSection title="Privacy & Permissions">
            ...
            <SheetTrigger asChild>
              <div className="cursor-pointer">
                <SettingsItem
                  ...
                  description={locationAccessDescriptions[locationAccess]}
                >
                  <ChevronRight className="h-5 w-5 text-muted-foreground" />
                </SettingsItem>
              </div>
          ...
          <SettingsItem icon={Camera} title="Camera permissions">
            <Switch
              ...
              onCheckedChange={handleCameraPermissionToggle}
            />
          </SettingsItem>
              ...
              <SheetFooter className="grid grid-cols-2 gap-4 pt-4">
                {cameraPermission ? (
                  <>
                    ...
                    <Button
                      variant="destructive"
                      onClick={() => handleCameraPermissionConfirm(false)}
                    >
                      Disable
                    </Button>
                  </>
                  ...
                  <>
                    ...
                    <Button onClick={() => handleCameraPermissionConfirm(true)}>
                      Allow
                    </Button>
                  </>
                )}
          ...
          <SettingsItem
            icon={Database}
            title="Data usage"
            description="Opt-out of analytics for app improvement"
          >
            <Switch
          ...
          <Sheet
            open={isDataUsageSheetOpen}
            onOpenChange={setIsDataUsageSheetOpen}
          >
            <SheetContent side="bottom" className="rounded-t-2xl">
              <SheetHeader className="text-left">
              ...
              <SheetFooter className="grid grid-cols-2 gap-4 pt-4">
                <Button
                  variant="outline"
                  onClick={() => handleDataUsageConfirm(false)}
                >
                  No
                </Button>
                <Button onClick={() => handleDataUsageConfirm(true)}>
                  Yes
                </Button>
              </SheetFooter>
            </SheetContent>
