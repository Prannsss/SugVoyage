            ...
            {[1, 2, 3, 4, 5].map((star) => (
                <Star
                    ...
                    className={cn(
                        ...
                        star <= rating ? "text-accent fill-accent" : "text-muted-foreground/50"
                    )}
...
export default function NewPostPage() {
  ...
  const handleSubmit = (event: FormEvent) => {
    ...
    console.log('Submitting new post:', { caption, files, privacy, location, rating });
          ...
          <CardContent className="p-4 space-y-4">
             <div className="flex items-start gap-4">
                <Avatar className="mt-1">
                    <AvatarImage src="https://picsum.photos/100/100?random=99" alt="User" />
            ...
            <div className="flex items-center justify-between border-y py-2">
                 <div className="flex items-center">
                    <Button type="button" variant="ghost" size="icon" className="text-muted-foreground" onClick={triggerFileSelect} aria-label="Add Photo or Video">
                        <ImageIcon className="h-6 w-6" />
                    </Button>
                    <Sheet open={isTaggingSheetOpen} onOpenChange={setIsTaggingSheetOpen}>
                        <SheetTrigger asChild>
                            <Button type="button" variant="ghost" size="icon" className="text-muted-foreground" aria-label="Tag People">
                        ...
                        <SheetContent side="top" variant="top-minimal">
                            ...
                            <div className="py-4">
                                <Label htmlFor="tag-input" className="sr-only">Tag People</Label>
                                <Input 
                                    id="tag-input"
                            ...
                            <SheetFooter>
                                <SheetClose asChild>
                                    <Button type="button" onClick={() => setIsTaggingSheetOpen(false)}>Done</Button>
                                </SheetClose>
                            </SheetFooter>
                        </SheetContent>
                    </Sheet>
                 ...
                 <div className="flex items-center gap-1">
                    <Sheet open={isLocationSheetOpen} onOpenChange={setIsLocationSheetOpen}>
                        <SheetTrigger asChild>
                             <Button type="button" variant={location ? "secondary" : "ghost"} size="icon" className={cn(location && "text-primary")}>
                        ...
                        <SheetContent side="top" variant="top-minimal">
                            ...
                            <SheetFooter>
                                <SheetClose asChild>
                                    <Button type="button" onClick={() => setIsLocationSheetOpen(false)}>Done</Button>
                                </SheetClose>
                            </SheetFooter>
                        </SheetContent>
                    </Sheet>
                    ...
                    <Sheet open={isRatingSheetOpen} onOpenChange={setIsRatingSheetOpen}>
                        <SheetTrigger asChild>
                             <Button type="button" variant={rating > 0 ? "secondary" : "ghost"} size="icon" className={cn(rating > 0 && "text-accent")}>
                        ...
                        <SheetContent side="top" variant="top-minimal">
                            ...
                            <StarRating rating={rating} setRating={setRating} />
                             <SheetFooter>
                                <SheetClose asChild>
                                    <Button type="button" onClick={() => setIsRatingSheetOpen(false)}>Done</Button>
                                </SheetClose>
                            </SheetFooter>
                        </SheetContent>
                    </Sheet>
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button type="button" variant="ghost" size="icon" aria-label="Privacy Settings">
                ...
                {previews.map((preview, index) => {
                  ...
                  return (
                    <div key={index} className="relative aspect-square">
                      {isVideo ? (
                         <video src={preview} className="w-full h-full object-cover rounded-md bg-muted" controls={false} />
                      ) : (
                        <Image
                          src={preview}
                          alt={`Preview ${index + 1}`}
                      ...
                      {isVideo && <Video className="absolute bottom-1 left-1 h-5 w-5 text-white drop-shadow-md" />}
                    </div>
                  );
                })}
                 <div 
                    className={cn(
                      "flex flex-col items-center justify-center text-center rounded-md border-2 border-dashed cursor-pointer hover:border-primary transition-colors aspect-square",
                       (files.filter(f => f.type.startsWith('image/')).length >= MAX_IMAGES && files.filter(f => f.type.startsWith('video/')).length >= MAX_VIDEOS) && "hidden"
                    )}
                  ...
                  >
                      ...
                      <p className="text-xs mt-1 text-muted-foreground">Add Media</p>
                  </div>
              </div>
            )}
          </CardContent>
