...
import { Languages, Bookmark, Link as LinkIcon, ShieldAlert, AlertCircle } from 'lucide-react';
...
export function PostOptionsSheet({ isOpen, onOpenChange, post }: PostOptionsSheetProps) {
  ...
  const isCurrentUserPost = post.user.username === 'alex_doe';
    ...
    <Sheet open={isOpen} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="flex-1 space-y-2">
            ...
            <Button variant="ghost" className="w-full justify-start h-12 text-base" onClick={handleCopyLink}>
                <LinkIcon className="mr-4" /> Copy Link
            </Button>
            <Separator className="my-2" />
            {!isCurrentUserPost && (
                <>
                     <Button variant="ghost" className="w-full justify-start h-12 text-base text-destructive hover:text-destructive">
                        <ShieldAlert className="mr-4" /> Block
                    </Button>
                     <Button variant="ghost" className="w-full justify-start h-12 text-base text-destructive hover:text-destructive" onClick={handleReport}>
